...
public class ChatReportBuilder {
   ...
   private ChatReportBuilder(UUID pReportId, Instant pCreatedAt, UUID pReportedProfileId, AbuseReportLimits pLimits) {
   ...
   public ChatReportBuilder(UUID pReportedProfileId, AbuseReportLimits pLimits) {
      this(UUID.randomUUID(), Instant.now(), pReportedProfileId, pLimits);
   }
   ...
   public boolean isReported(int pId) {
      return this.reportedMessages.contains(pId);
   }
   ...
   private ReportEvidence buildEvidence(ChatLog pChatLog) {
      ...
      this.reportedMessages.forEach((p_242060_) -> {
         Int2ObjectMap<LoggedChatMessage.Player> int2objectmap = collectReferencedContext(pChatLog, p_242060_, this.limits);
         ...
         for(UUID uuid : set) {
            this.chainForPlayer(pChatLog, int2objectmap, uuid).forEach((p_242067_) -> {
   ...
   private Stream<ChatLog.Entry<LoggedChatMessageLink>> chainForPlayer(ChatLog pChatLog, Int2ObjectMap<LoggedChatMessage.Player> p_242153_, UUID p_242301_) {
      ...
      return pChatLog.selectBetween(i, j).entries().map((p_242069_) -> {
   ...
   private static Int2ObjectMap<LoggedChatMessage.Player> collectReferencedContext(ChatLog pChatLog, int p_242178_, AbuseReportLimits p_242421_) {
      ...
      walkMessageReferenceGraph(pChatLog, p_242178_, (p_242693_, p_242694_) -> {
         int2objectmap.put(p_242693_, p_242694_);
         return int2objectmap.size() < i;
      });
      trailingContext(pChatLog, p_242178_, p_242421_.trailingContextMessageCount()).forEach((p_242057_) -> {
         int2objectmap.put(p_242057_.id(), p_242057_.event());
      });
      return int2objectmap;
   }

   private static Stream<ChatLog.Entry<LoggedChatMessage.Player>> trailingContext(ChatLog pChatLog, int p_242340_, int p_242471_) {
      return pChatLog.selectAfter(pChatLog.after(p_242340_)).entries().map((p_242065_) -> {
         return p_242065_.tryCast(LoggedChatMessage.Player.class);
      }).filter(Objects::nonNull).limit((long)p_242471_);
   }

   private static void walkMessageReferenceGraph(ChatLog pChatLog, int p_242234_, ChatReportBuilder.ReferencedMessageVisitor pVisitor) {
      ...
      while(!intpriorityqueue.isEmpty()) {
         ...
         if (loggedchatevent instanceof LoggedChatMessage.Player loggedchatmessage$player) {
            if (!pVisitor.accept(i, loggedchatmessage$player)) {
               break;
            }

            for(int j : messageReferences(pChatLog, i, loggedchatmessage$player.message())) {
               if (intset.add(j)) {
   ...
   private ReportChatMessage buildReportedChatMessage(int pChatId, LoggedChatMessage.Player pLoggedPlayerChatMessage) {
      PlayerChatMessage playerchatmessage = pLoggedPlayerChatMessage.message();
      ...
      return new ReportChatMessage(new ReportChatMessageHeader(bytebuffer1, pLoggedPlayerChatMessage.profileId(), bytebuffer2, bytebuffer), new ReportChatMessageBody(instant, i, list, reportchatmessagecontent), s, this.isReported(pChatId));
   }
