...
public class ChatSelectionScreen extends Screen {
   ...
   public class ChatSelectionList extends ObjectSelectionList<ChatSelectionScreen.ChatSelectionList.Entry> implements ChatSelectionLogFiller.Output<LoggedChatMessage.Player> {
      ...
      public void acceptMessage(int pChatId, LoggedChatMessage.Player pLoggedChatMessage) {
         boolean flag = pLoggedChatMessage.canReport(ChatSelectionScreen.this.report.reportedProfileId());
         ChatTrustLevel chattrustlevel = pLoggedChatMessage.trustLevel();
         GuiMessageTag guimessagetag = chattrustlevel.createTag(pLoggedChatMessage.message());
         ChatSelectionScreen.ChatSelectionList.Entry chatselectionscreen$chatselectionlist$entry = new ChatSelectionScreen.ChatSelectionList.MessageEntry(pChatId, pLoggedChatMessage.toContentComponent(), pLoggedChatMessage.toNarrationComponent(), guimessagetag, flag, true);
         this.addEntryToTop(chatselectionscreen$chatselectionlist$entry);
         this.updateHeading(pLoggedChatMessage, flag);
      }
      ...
      public class MessageEntry extends ChatSelectionScreen.ChatSelectionList.Entry {
         ...
         public MessageEntry(int pChatId, Component pText, @Nullable Component pNarration, GuiMessageTag pTagIcon, boolean pCanReport, boolean pPlayerMessage) {
            this.chatId = pChatId;
            this.tagIcon = Util.mapNullable(pTagIcon, GuiMessageTag::icon);
         ...
         private void renderTag(PoseStack pPoseStack, int pX, int pY, int pHeight, int pMouseX, int pMouseY) {
         ...
         private void renderSelectedCheckmark(PoseStack pPoseStack, int pTop, int pLeft, int pHeight) {
            int i = pTop + (pHeight - 8) / 2;
            RenderSystem.setShaderTexture(0, CHECKMARK_TEXTURE);
            RenderSystem.enableBlend();
            GuiComponent.blit(pPoseStack, pLeft, i, 0.0F, 0.0F, 9, 8, 9, 8);
      ...
      public class MessageHeadingEntry extends ChatSelectionScreen.ChatSelectionList.Entry {
         ...
         private void renderFace(PoseStack pPoseStack, int pX, int pY, ResourceLocation pSkinLocation) {
            RenderSystem.setShaderTexture(0, pSkinLocation);
