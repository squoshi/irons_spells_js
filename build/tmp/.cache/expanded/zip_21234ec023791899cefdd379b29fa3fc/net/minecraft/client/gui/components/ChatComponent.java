...
public class ChatComponent extends GuiComponent {
   ...
   private void drawTagIcon(PoseStack pPoseStack, int pLeft, int pBottom, GuiMessageTag.Icon pTagIcon) {
      int i = pBottom - pTagIcon.height - 1;
      pTagIcon.draw(pPoseStack, pLeft, i);
   }
   ...
   private void logChatMessage(Component pChatComponent, @Nullable GuiMessageTag pTag) {
      ...
      String s1 = Util.mapNullable(pTag, GuiMessageTag::logTag);
      if (s1 != null) {
   ...
   public void deleteMessage(MessageSignature pMessageSignature) {
      ...
      while(iterator.hasNext()) {
         ...
         if (messagesignature != null && messagesignature.equals(pMessageSignature)) {
            iterator.remove();
            break;
         }
      }
   ...
   private int getMessageIndexAt(double pChatY) {
      if (this.isChatFocused() && !this.minecraft.options.hideGui && !this.isChatHidden()) {
         int i = Math.min(this.getLinesPerPage(), this.trimmedMessages.size());
         if (pChatY >= 0.0D && pChatY < (double)(9 * i + i)) {
            int j = Mth.floor(pChatY / 9.0D + (double)this.chatScrollbarPos);
