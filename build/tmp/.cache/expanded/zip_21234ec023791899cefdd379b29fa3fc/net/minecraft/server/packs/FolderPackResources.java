...
public class FolderPackResources extends AbstractPackResources {
   ...
   public static boolean validatePath(File pPath, String pFilename) throws IOException {
      String s = pPath.getCanonicalPath();
      if (ON_WINDOWS) {
         s = BACKSLASH_MATCHER.replaceFrom(s, '/');
      }

      return s.endsWith(pFilename);
   }
   ...
   private File getFile(String pFilename) {
      try {
         File file1 = new File(this.file, pFilename);
         if (file1.isFile() && validatePath(file1, pFilename)) {
            return file1;
         }
   ...
   private void listResources(File pDirectory, String pNamespace, List<ResourceLocation> p_215336_, String p_215337_, Predicate<ResourceLocation> p_215338_) {
      File[] afile = pDirectory.listFiles();
      if (afile != null) {
         for(File file1 : afile) {
            if (file1.isDirectory()) {
               this.listResources(file1, pNamespace, p_215336_, p_215337_ + file1.getName() + "/", p_215338_);
               ...
               try {
                  ...
                  if (resourcelocation == null) {
                     LOGGER.warn("Invalid path in datapack: {}:{}, ignoring", pNamespace, s);
