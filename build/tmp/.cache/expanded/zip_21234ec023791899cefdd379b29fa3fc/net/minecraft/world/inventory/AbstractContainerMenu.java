   ...
   private void doClick(int pSlotId, int pButton, ClickType pClickType, Player pPlayer) {
      ...
      if (pClickType == ClickType.QUICK_CRAFT) {
         ...
         if (pSlotId == -999) {
            ...
            for(ItemStack itemstack9 = this.quickMoveStack(pPlayer, pSlotId); !itemstack9.isEmpty() && ItemStack.isSame(slot6.getItem(), itemstack9); itemstack9 = this.quickMoveStack(pPlayer, pSlotId)) {
            }
         } else {
            if (pSlotId < 0) {
               return;
         ...
         ItemStack itemstack4 = inventory.getItem(pButton);
         ...
         int i1 = pButton == 0 ? 1 : slot4.getItem().getCount();
         ...
         if (!itemstack5.isEmpty() && (!slot3.hasItem() || !slot3.mayPickup(pPlayer))) {
            int k1 = pButton == 0 ? 0 : this.slots.size() - 1;
            int j2 = pButton == 0 ? 1 : -1;
