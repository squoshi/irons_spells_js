...
public class InteractWith<E extends LivingEntity, T extends LivingEntity> extends Behavior<E> {
   ...
   public static <T extends LivingEntity> InteractWith<LivingEntity, T> of(EntityType<? extends T> pType, int pInteractionRange, Predicate<T> pTargetFilter, MemoryModuleType<T> pMemory, float pSpeedModifier, int pMaxDist) {
      return new InteractWith<>(pType, pInteractionRange, (p_147584_) -> {
         return true;
      }, pTargetFilter, pMemory, pSpeedModifier, pMaxDist);
   }
   ...
   private boolean seesAtLeastOneValidTarget(E pEntity) {
      NearestVisibleLivingEntities nearestvisiblelivingentities = pEntity.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).get();
   ...
   private boolean canInteract(E pEntity, LivingEntity pTarget) {
      return this.type.equals(pTarget.getType()) && pTarget.distanceToSqr(pEntity) <= (double)this.interactionRangeSqr && this.targetFilter.test((T)pTarget);
   }
}