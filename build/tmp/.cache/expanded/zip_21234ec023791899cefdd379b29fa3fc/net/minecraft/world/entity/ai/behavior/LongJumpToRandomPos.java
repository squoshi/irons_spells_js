...
public class LongJumpToRandomPos<E extends Mob> extends Behavior<E> {
   ...
   protected boolean isAcceptableLandingPosition(ServerLevel pLevel, E pEntity, BlockPos pPos) {
      ...
      int i = blockpos.getX();
      int j = blockpos.getZ();
      if (i == pPos.getX() && j == pPos.getZ()) {
         return false;
      } else if (!pEntity.getNavigation().isStableDestination(pPos) && !this.acceptableLandingSpot.test(pLevel.getBlockState(pPos.below()))) {
         return false;
      } else {
         return pEntity.getPathfindingMalus(WalkNodeEvaluator.getBlockPathTypeStatic(pEntity.level, pPos.mutable())) == 0.0F;
      }
   }
   ...
   @Nullable
   private Vec3 calculateJumpVectorForAngle(Mob pMob, Vec3 p_217308_, int p_217309_) {
      ...
      if (d11 < 0.0D) {
         ...
         if (d12 > (double)this.maxJumpVelocity) {
            ...
            for(int j = 0; j < i - 1; ++j) {
               ...
               if (vec33 != null && !this.isClearTransition(pMob, vec33, vec34)) {
                  return null;
               }
   ...
   private boolean isClearTransition(Mob pMob, Vec3 p_147665_, Vec3 p_147666_) {
      ...
      for(int j = 0; j < i; ++j) {
         ...
         if (!pMob.level.noCollision(pMob, aabb)) {
            return false;
         }
      }
